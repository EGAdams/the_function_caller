...
# Goal
- Debug the unit test failures for the tool: `{ tool_name }`
...
# Your Task
Create more, smaller unit tests because the ones that we have are to involved in too many things.  Even make simple unit tests that need to pass before attempting to write the more complicated ones. Rewrite the test file `{ test_file }` to conform to this more simple approach.
"""   
    return prompt
